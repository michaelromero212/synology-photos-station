...
| **M2** | Import existing library | Walk `/photo` + `/homes/*/Photos`, populate everything. Overnight run. **This is what makes the app non-empty.** |
| **M3** 🟡 | Timeline | **Server done** — manifest at year/month/day zoom, per-bucket items, delta sync, asset detail with camera card + attribution. 34 assertions green. **Client partial** — sectioned grid with ThumbHash placeholders, two-tier thumbnail cache, Keychain credentials, space switcher. **Remaining:** Information panel UI, `UICollectionView` swap for 100k scale, offline reverse geocode. |
