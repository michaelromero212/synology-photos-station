...
OUT=$(rebuild --dry-run)
check "it sees four files" "1" "$(echo "$OUT" | grep -c 'Found:         4 files')"
check "and both people" "1" "$(echo "$OUT" | grep -c 'People:        michael, morgan')"
...
echo "=== 2. rebuild ==="
OUT=$(rebuild)
check "indexed four" "1" "$(echo "$OUT" | grep -c 'Indexed: 4')"
check "failed none" "1" "$(echo "$OUT" | grep -c 'Failed:  0')"
check "four assets" "4" "$(q "select count(*) from assets;")"
check "four placements" "4" "$(q "select count(*) from space_assets;")"

echo
echo "=== 3. the people came back ==="
check "two accounts" "2" "$(q "select count(*) from users;")"
check "named after their homes" "michael,morgan" "$(q "select string_agg(dsm_username,',' order by dsm_username) from users;")"
...
check "one personal library each" "2" "$(q "select count(*) from spaces where kind='personal';")"
check "each owned by its person" "2" "$(q "select count(*) from space_members m join spaces s on s.id=m.space_id where s.kind='personal' and m.role='owner' and s.created_by=m.user_id;")"
...
check "michael has two" "2" "$(mine michael)"
...
check "every asset points at a file" "4" "$(q "select count(*) from assets where storage_path is not null;")"
...
check "browse_path is that same file" "4" "$(q "select count(*) from space_assets sa join assets a on a.id=sa.asset_id where sa.browse_path = a.storage_path;")"
...
check "everything has a date to sort by" "4" "$(q "select count(*) from assets where captured_at is not null;")"
check "and a local time to bucket on" "4" "$(q "select count(*) from assets where local_captured_at is not null;")"
...
check "one change per photo" "4" "$(q "select count(*) from change_log where op='insert';")"
check "thumbnails queued" "4" "$(q "select count(*) from derivation_jobs where kind='thumbnails';")"
...
check "still four assets" "4" "$(q "select count(*) from assets;")"
check "still four placements" "4" "$(q "select count(*) from space_assets;")"
check "still two people" "2" "$(q "select count(*) from users;")"
...
check "five assets now" "5" "$(q "select count(*) from assets;")"
check "michael has three" "3" "$(mine michael)"
